...
### How ```run_analysis.R```works + Transformation details
...
7. Create a second, independent tidy data set with the average of each variable for each activity and each subject, using the data from the previous step. The final tidy dataset is saved in `tidydata.txt` 
